...
- **Variables disponibles al momento de predecir**: aquí encontré algo que no esperaba. Al abrir el CSV real me aparecieron **16 columnas**, no las 13 que documenta oficialmente la página de UCI. Las tres columnas extra (`Age`, `FN`, `FP`) no están en la tabla de variables oficial del repositorio, así que no puedo confiar en que estén disponibles o bien definidas en un escenario real de predicción. Las documento en el diccionario de datos (`docs/diccionario_datos.md`) y las excluyo en el paso 7, con la razón específica de cada una.
- **Tamaño compatible con CPU/Colab gratuito**: cumple sin problema. Son 3,150 filas × 16 columnas (~200 KB en memoria), entrena en segundos en CPU.
